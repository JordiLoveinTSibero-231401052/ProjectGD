...
if (!$team_id) {
    ...
    if ($team_response === false || $http_code !== 200) {
        ...
        if (isset($team_data['id'])) {
            ...
            // PostgreSQL query to check if the team is a favorite
            $stmt = $pdo->prepare("SELECT id FROM favorites WHERE user_id = :user_id AND team_id = :team_id");
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                if ($is_favorite) {
                    // PostgreSQL query to remove from favorites
                    $stmt = $pdo->prepare("DELETE FROM favorites WHERE user_id = :user_id AND team_id = :team_id");
                    $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
                    $stmt->bindParam(':team_id', $team_id, PDO::PARAM_INT);
                    if (!$stmt->execute()) {
                    ...
                    // PostgreSQL query to add to favorites
                    $stmt = $pdo->prepare("INSERT INTO favorites (user_id, team_id, team_name, team_logo) VALUES (:user_id, :team_id, :team_name, :team_logo)");
                    $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
                    $stmt->bindParam(':team_id', $team_id, PDO::PARAM_INT);
                    $stmt->bindParam(':team_name', $team_name, PDO::PARAM_STR);
                    $stmt->bindParam(':team_logo', $team_logo, PDO::PARAM_STR);
                    if (!$stmt->execute()) {
