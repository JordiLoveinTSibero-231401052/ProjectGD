...
// Check if the form is submitted to remove a favorite
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['favorite_id'])) {
    $favorite_id = intval($_POST['favorite_id']);

    // PostgreSQL query to delete a favorite
    $stmt = $pdo->prepare("DELETE FROM favorites WHERE id = :favorite_id AND user_id = :user_id");
...
// Query to fetch favorite teams
$query = "SELECT id, team_id, team_name, team_logo FROM favorites WHERE user_id = :user_id";
